...
function getDecoration (ele) {
  ...
  if (/dp-highlighter/.test(ele.className)) {
    var lis = ele.querySelectorAll('ol li')
    let text = ''

    // get language (for CSDN blog)
    let lang = ele.querySelector('.tools')
    if (lang) lang = /^\s*\[(\w+)\]/.exec(lang.textContent)
    lang = lang ? lang[1] : ''

    for (let i = 0; i < lis.length; i++) text += lis[i].textContent.replace(/\s+$/, '') + '\n'
    return { start: '\n\n```' + lang + '\n' + text + '```\n\n', skip: true }
  }
  ...
  if (tagName === 'code' && ele.parentElement.tagName !== 'PRE') return { start: '`' + ele.textContent + '`', skip: true }
  if (tagName === 'blockquote') return { start: '\n\n', end: '\n\n', lead: '> ' }
  if (tagName === 'br') return { start: '\n' }

  if (tagName === 'table') return { start: '\n\n', end: '\n\n' }
  if (tagName === 'tr') {
    var end = '\n'
    if (
  ...
  if (tagName === 'li') {
    var indent = ele.getAttribute(LI_ATTR_INDENT) - 1
    var index = ele.getAttribute(LI_ATTR_INDEX) || '-'
    index = ' ' + index + ' '
